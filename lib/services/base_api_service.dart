...
class BaseApiService {
  ...
  // Session-level verified URL — skips health check once confirmed working
  static String? _verifiedUrl;
  static DateTime? _verifiedAt;
  static const _verifiedTTL = Duration(minutes: 10);
  ...
  ///
  /// Uses a session-level cache (10-minute TTL) so the health-check
  /// HTTP call is only made ONCE per session, not on every API request.
  ///
  /// Discovery strategy (only runs when cache is empty/expired):
  /// 1. Try hardcoded URL from ApiConfig (production/cloud server)
  /// 2. Check cached URL in SharedPreferences
  /// 3. Try mDNS discovery for _http._tcp.local services
  /// 4. Try .local hostname from .env (BACKEND_HOSTNAME)
  /// 5. Fallback to BACKEND_URL from .env
  /// 6. Throw exception if nothing works
  Future<String> getBackendUrl({Duration timeout = const Duration(seconds: 4)}) async {
    // Fast path: return session-verified URL if still fresh
    if (_verifiedUrl != null &&
        _verifiedAt != null &&
        DateTime.now().difference(_verifiedAt!) < _verifiedTTL) {
      return _verifiedUrl!;
    }

    // If ApiConfig has a hardcoded URL, trust it directly (production).
    // Skip the health check entirely — the server is known and remote.
    final configUrl = ApiConfig.baseUrl;
    if (configUrl.isNotEmpty) {
      _save(configUrl);
      _verifiedUrl = configUrl;
      _verifiedAt = DateTime.now();
      return configUrl;
    }

    // No hardcoded URL — do full discovery (local dev / dynamic IP)
    // 1) Check saved preference
    if (_cachedUrl != null) {
      if (await _isReachable(_cachedUrl!, timeout: timeout)) {
        _verifiedUrl = _cachedUrl;
        _verifiedAt = DateTime.now();
        return _cachedUrl!;
      }
    }
  ...
  /// Invalidate the session cache (call this if you know the backend URL changed)
  ...
  Future<http.Response> get(String path, {Map<String, String>? headers}) async {
    ...
    return http.get(uri, headers: headers).timeout(ApiConfig.requestTimeout);
  }
  ...
  Future<http.Response> post(String path, {Map<String, String>? headers, Object? body}) async {
    final base = await getBackendUrl();
    final uri = Uri.parse(_concat(base, path));
    return http.post(uri, headers: headers, body: body).timeout(ApiConfig.requestTimeout);
  }
  ...
  Future<http.Response> put(String path, {Map<String, String>? headers, Object? body}) async {
    final base = await getBackendUrl();
    final uri = Uri.parse(_concat(base, path));
    return http.put(uri, headers: headers, body: body).timeout(ApiConfig.requestTimeout);
  }
  ...
  Future<http.Response> delete(String path, {Map<String, String>? headers}) async {
    final base = await getBackendUrl();
    final uri = Uri.parse(_concat(base, path));
    return http.delete(uri, headers: headers).timeout(ApiConfig.requestTimeout);
  }
