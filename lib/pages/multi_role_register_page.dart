...
class _MultiRoleRegisterPageState extends State<MultiRoleRegisterPage> {
  ...
  final _doctorsCountController = TextEditingController();

  // Client medical information
  String? _selectedBloodGroup;
  bool _hasMedicalAllergies = false;
  final _emergencyContactController = TextEditingController();
  final _bloodDonorPhoneController = TextEditingController();
  ...
  void dispose() {
    ...
    _icuCountController.dispose();
    _bedsCountController.dispose();
    _doctorsCountController.dispose();
    _emergencyContactController.dispose();
    _bloodDonorPhoneController.dispose();
      ...
      switch (_selectedRole) {
        case UserRole.client:
          ...
          result = await _authService.registerClient(
            ...
            bloodDonorPhone: _bloodDonorPhoneController.text.trim(),
        ...
        case UserRole.admin:
          result = await _authService.registerHospital(
            ...
            icuCount: int.tryParse(_icuCountController.text.trim()) ?? 0,
            bedsCount: int.tryParse(_bedsCountController.text.trim()) ?? 0,
            doctorsCount: int.tryParse(_doctorsCountController.text.trim()) ?? 0,
          );
          break;
      }
  ...
  List<Widget> _buildClientFields() {
    return [
      ...
      // Blood Donor Phone
      TextFormField(
        controller: _bloodDonorPhoneController,
        decoration: const InputDecoration(
          labelText: 'Blood Donor Phone Number',
          prefixIcon: Icon(Icons.phone_in_talk),
          border: OutlineInputBorder(),
          hintText: 'Enter blood donor contact',
        ),
        keyboardType: TextInputType.phone,
        validator: (value) {
          if (value != null && value.isNotEmpty) {
      ...
      TextFormField(
        ...
        decoration: const InputDecoration(
          labelText: 'Medical Allergies',
          prefixIcon: Icon(Icons.medical_information),
          border: OutlineInputBorder(),
          hintText: 'Enter any allergies (e.g., penicillin, peanuts)',
        ),
        maxLines: 3,
  ...
  List<Widget> _buildHospitalFields() {
    return [
      ...
      const SizedBox(height: 16),
      TextFormField(
        controller: _icuCountController,
        decoration: const InputDecoration(
          labelText: 'ICU Count *',
          prefixIcon: Icon(Icons.medical_services),
          border: OutlineInputBorder(),
          hintText: 'Enter number of ICU beds',
        ),
        keyboardType: TextInputType.number,
        validator: (v) {
          if (v == null || v.trim().isEmpty) return 'ICU count required';
      ...
      TextFormField(
        ...
        keyboardType: TextInputType.number,
        validator: (v) {
          if (v == null || v.trim().isEmpty) return 'Beds count required';
          if (int.tryParse(v) == null) return 'Enter valid number';
          return null;
        },
      ),
      const SizedBox(height: 16),
      TextFormField(
        controller: _doctorsCountController,
        decoration: const InputDecoration(
        ...
        keyboardType: TextInputType.number,
        validator: (v) {
          if (v == null || v.trim().isEmpty) return 'Doctors count required';
          if (int.tryParse(v) == null) return 'Enter valid number';
          return null;
        },
      ),
