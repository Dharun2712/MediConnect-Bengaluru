...
def accept_sos_request():
    ...
    # Notify patient (client) about driver acceptance
    ...
    # NOTIFY HOSPITALS IMMEDIATELY when driver accepts
    ...
    hospital_notification_payload = {
        ...
        "driver_contact": driver_contact,
        "vehicle": vehicle if isinstance(vehicle, str) else f"{vehicle.get('type', 'Ambulance')} ({vehicle.get('plate', '')})",
        "patient_name": sos_req.get("user_name", "Aswanth"),
        ...
        "status": "enroute",
        "timestamp": datetime.datetime.utcnow().isoformat(),
        "message": f"Driver {driver_name} is en route to patient {sos_req.get('user_name', 'Patient')}"
    }
    
    # Broadcast to all hospitals and admin room with MULTIPLE events for reliability
    socketio.emit('driver_accepted', hospital_notification_payload, room='admin')
...
def submit_injury_assessment():
    ...
    assessment_data = {
        ...
        "patient_name": sos_req.get("user_name", "Arun"),
